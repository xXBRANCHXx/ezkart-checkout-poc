...
(() => {
  ...
  async function prepare({ templateId, productId, brandName, products }) {
    const template = (await list()).find((item) => item.id === templateId);
    if (!template) throw Error("Choose an available template.");
    const product = products.find((item) => item.id === productId);
    const recipe = materialize(
      await json(template.recipeUrl),
      context(product, brandName),
    );
    const ids = new Set(),
      commerce = [];
    const check = (node) => {
    ...
    if (
      commerce.some(
        (node) => node.part !== "cart" && node.productId !== product.id,
      )
    )
  ...
  function attach(form, getProducts) {
    ...
    settings.innerHTML =
      '<label><span>Store name</span><input name="template_brand" maxlength="80" autocomplete="organization" placeholder="Your store name"></label><label><span>Featured product</span><select name="template_product"><option value="">Choose a product</option></select></label><p>Your product, photos and prices replace the demo content. Every section stays editable.</p><button type="button" data-template-preview>Preview design</button>';
  ...
  async function fromForm(form, products) {
    ...
    return prepare({
      templateId,
      productId: form.elements.template_product.value,
