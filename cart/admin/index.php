...
<?php else: ?>
  <svg class="svg-sprite" aria-hidden="true">
    ...
    <symbol id="icon-rocket" viewBox="0 0 24 24"><g fill="currentColor" stroke="none" transform="scale(.09375)"><path d="M152,224a8,8,0,0,1-8,8H112a8,8,0,0,1,0-16h32A8,8,0,0,1,152,224ZM128,112a12,12,0,1,0-12-12A12,12,0,0,0,128,112Zm95.62,43.83-12.36,55.63a16,16,0,0,1-25.51,9.11L158.51,200h-61L70.25,220.57a16,16,0,0,1-25.51-9.11L32.38,155.83a16.09,16.09,0,0,1,3.32-13.71l28.56-34.26a123.07,123.07,0,0,1,8.57-36.67c12.9-32.34,36-52.63,45.37-59.85a16,16,0,0,1,19.6,0c9.34,7.22,32.47,27.51,45.37,59.85a123.07,123.07,0,0,1,8.57,36.67l28.56,34.26A16.09,16.09,0,0,1,223.62,155.83ZM99.43,184h57.14c21.12-37.54,25.07-73.48,11.74-106.88C156.55,47.64,134.49,29,128,24c-6.51,5-28.57,23.64-40.33,53.12C74.36,110.52,78.31,146.46,99.43,184Zm-15,5.85Q68.28,160.5,64.83,132.16L48,152.36,60.36,208l.18-.13ZM208,152.36l-16.83-20.2q-3.42,28.28-19.56,57.69l23.85,18,.18.13Z"/></g></symbol>
    ...
    <symbol id="icon-templates" viewBox="0 0 24 24"><path d="M8 3h11a2 2 0 0 1 2 2v11"/><rect x="3" y="7" width="14" height="14" rx="2"/><path d="M3 12h14M9 12v9"/></symbol>
    ...
    <symbol id="icon-eye-off" viewBox="0 0 24 24"><path d="M3 12s3.5-6 9-6c2 0 3.8.8 5.2 1.8M21 12s-3.5 6-9 6c-2 0-3.8-.8-5.2-1.8M9.9 9.9a3 3 0 0 0 4.2 4.2M3 3l18 18"/></symbol>
    ...
    <div class="workspace">
      ...
      <main class="dashboard page-canvas" id="overview">
        ...
        <section class="sales-performance-heading" aria-labelledby="sales-performance-title">
          ...
          <form class="page-actions dashboard-period" method="get"><input type="hidden" name="page" value="dashboard"><input type="hidden" name="group" value="<?= ez_admin_escape($chartGroup) ?>"><label><?= ez_admin_icon('calendar') ?><select name="range" aria-label="Dashboard date range"><?php foreach (['7' => 'Last 7 days', '30' => 'Last 30 days', '90' => 'Last 90 days', 'all' => 'All time'] as $value => $label): ?><option value="<?= $value ?>" <?= $dashboardPeriod['range'] === (string) $value ? 'selected' : '' ?>><?= $label ?></option><?php endforeach; ?></select></label><button class="ui-button" type="submit" data-ui-icon="check">Apply</button></form>
        </section>

        <p class="dashboard-data-note"><?= $dateRangeStart !== null ? ez_admin_escape($dateRangeStart->format('j M Y') . ' – ' . $nowJakarta->format('j M Y') . ' · ') : 'All time · ' ?><?= $commerceProduction ? 'Production' : 'Sandbox' ?> order records · Updated <?= $nowJakarta->format('H:i') ?> WIB <a href="<?= ez_admin_escape('?' . http_build_query(['page' => 'dashboard', 'range' => $dashboardPeriod['range'], 'group' => $chartGroup])) ?>" data-ui-icon="refresh">Refresh</a></p>
        ...
        <section class="dashboard-grid primary-grid">
          ...
          <article class="panel products-panel" id="top-products">
            <header class="panel-header"><h2>Top Selling Products</h2><a href="?page=products" data-ui-icon="eye">View all</a></header>
        ...
        <section class="dashboard-grid secondary-grid">
          ...
          <article class="panel revenue-panel"><header class="panel-header"><h2>Revenue Breakdown</h2><a href="?page=payments" data-ui-icon="chart">View Report</a></header><div class="revenue-rows">
        ...
        <section class="dashboard-grid footer-grid">
          ...
          <article class="panel payout-panel" id="payout-summary"><header class="panel-header"><h2>Payment Summary</h2><a href="?page=payments" data-ui-icon="credit-card">View all payments</a></header><div class="payout-body"><small>Provider-confirmed Volume</small><div><strong><?= ez_admin_money($metrics['paid_volume']) ?></strong><?php if ($metrics['paid_count'] > 0): ?><em class="positive"><?= ez_admin_icon('check-circle') ?> Confirmed</em><?php endif; ?><span><?= $metrics['paid_count'] > 0 ? 'provider-confirmed payments' : 'No confirmed payments in this period' ?></span></div></div><footer><div><small>Environment</small><b><?= $commerceProduction ? 'DOKU production' : 'DOKU sandbox' ?></b></div><div><small>Paid Orders</small><b><?= number_format($metrics['paid_count']) ?></b></div><a href="../" data-ui-icon="external-link">Open Checkout</a></footer></article>
        </section>
        <?php endif; ?>
      </main>
