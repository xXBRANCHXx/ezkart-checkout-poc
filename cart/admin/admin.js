...
(async () => {
  ...
  if (sqStudio) {
    ...
    previewCropToggle?.addEventListener('change', () => {
      remember();
      previewRoot.classList.toggle('sq-overflow-visible', !previewCropToggle.checked);
      markSqChanged();
      livePreviewFrame.contentWindow?.postMessage({type:'ezkart-preview-crop', crop:previewCropToggle.checked}, '*');
    });
    ...
    sqStudio.querySelector("[data-sq-preview]")?.addEventListener("click", async () => {
      ...
      if (previewCropToggle) previewCropToggle.checked = !previewRoot.classList.contains('sq-overflow-visible');
