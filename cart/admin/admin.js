...
const positionFloatingCart=()=>{
 ...
 const controls=[...document.querySelectorAll('[data-native-scroll-visibility],[data-commerce-add],[data-commerce-set],[data-ezkart-add],.sq-native-commerce-options')].map(node=>({node,rect:node.getBoundingClientRect()})).filter(({node,rect})=>!node.closest('dialog')&&rect.width&&rect.height&&rect.bottom>0&&rect.top<innerHeight&&(!node.hasAttribute('data-native-scroll-visibility')||getComputedStyle(node).position==='fixed')).sort((a,b)=>b.rect.bottom-a.rect.bottom);
