...
document.querySelectorAll('[class*="animation-"],[class*="element-animation-"]').forEach(element=>observer.observe(element))
      ...
      const motionScripts = hasScrollMotion ? `<script>(()=>{const frames=[...document.querySelectorAll('.ezkart-scroll-frame')].map(frame=>({frame,media:frame.querySelector(':scope>.ezkart-scroll-media'),effect:frame.dataset.ezkartScrollEffect||'parallax',strength:Math.max(0,Math.min(100,Number(frame.dataset.ezkartScrollStrength)||0))/100})).filter(item=>item.media);if(!frames.length)return;const reduced=matchMedia('(prefers-reduced-motion: reduce)');let raf=0,viewportHeight=1;const clamp=value=>Math.max(0,Math.min(1,value));const render=()=>{raf=0;if(reduced.matches)return;frames.forEach(item=>{const rect=item.frame.getBoundingClientRect();if(rect.bottom<0||rect.top>viewportHeight)return;const progress=clamp((viewportHeight-rect.top)/(viewportHeight+rect.height));if(item.effect==='zoom'){item.media.style.setProperty('--ezkart-scroll-y','0px');item.media.style.setProperty('--ezkart-scroll-scale',String(1+progress*item.strength*.3));return}const reverse=item.effect==='parallax-reverse',rate=reverse?item.strength*.35:item.strength;const offset=(progress-.5)*(viewportHeight+rect.height)*rate*(reverse?-1:1);item.media.style.setProperty('--ezkart-scroll-y',offset.toFixed(3)+'px');item.media.style.setProperty('--ezkart-scroll-scale','1')})};const schedule=()=>{if(!raf)raf=requestAnimationFrame(render)};const measure=()=>{viewportHeight=Math.max(1,document.documentElement.clientHeight||innerHeight);frames.forEach(item=>{const frameHeight=Math.max(1,item.frame.clientHeight),reverse=item.effect==='parallax-reverse',rate=reverse?item.strength*.35:item.strength,overscan=reverse?(viewportHeight+frameHeight)*rate:Math.max(0,viewportHeight-frameHeight)*rate;const height=item.effect==='zoom'||reduced.matches?frameHeight:frameHeight+overscan;item.media.style.height=height.toFixed(3)+'px';if(reduced.matches){item.media.style.removeProperty('--ezkart-scroll-y');item.media.style.removeProperty('--ezkart-scroll-scale')}});schedule()};addEventListener('scroll',schedule,{passive:true});addEventListener('resize',measure,{passive:true});addEventListener('pageshow',measure);reduced.addEventListener?.('change',measure);if(typeof ResizeObserver==='function'){const observer=new ResizeObserver(measure);frames.forEach(item=>observer.observe(item.frame))}document.fonts?.ready.then(measure);measure()})();<\/script>` : "";
