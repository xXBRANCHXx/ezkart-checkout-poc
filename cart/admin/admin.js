...
addEventListener('resize',schedule);document.addEventListener('toggle',schedule,true);document.addEventListener('load',schedule,true);document.fonts?.ready.then(schedule);schedule();
      ...
      const extraFontCss = bundledFonts.filter(font=>clone.outerHTML.includes(font.family)).map(font=>`@font-face{font-family:'${font.family}';src:url('${exportFontData.get(font.key)||new URL('assets/fonts/'+font.file+'.woff2',exportBase).href}') format('woff2');font-weight:${font.weight};font-display:swap}`).join('');
