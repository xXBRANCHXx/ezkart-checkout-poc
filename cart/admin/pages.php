...
function ez_orders_table(array $rows, string $tableId, string $csrfToken): void
{
    ?>
...
<?php switch ($page): case 'orders': ?>
  <?php ez_page_header('Orders', 'Manage orders, payments, and deliveries.', [
      ['label' => 'Refresh data', 'href' => '?page=orders'], ['label' => 'Open checkout', 'href' => '../', 'style' => 'primary'],
  ]); ?>
  ...
  <section class="page-grid orders-control-grid">
    <article class="surface orders-workspace"><header class="surface-header"><div><h2>Order manager</h2><p>Search, filter, and inspect complete transaction records.</p></div><div class="surface-tools"><label class="surface-search"><?= ez_admin_icon('search') ?><input id="order-search" type="search" placeholder="Search orders, customers, products"></label><select id="status-filter"><option value="all">All statuses</option><option value="PAID">Paid</option><option value="PENDING">Pending</option><option value="CREATING">Creating</option><option value="FAILED">Failed</option></select></div></header><?php ez_orders_table($allDisplayOrders, 'order-list', $csrfToken); ?></article>
...
<?php break; case 'sites': require __DIR__ . ($siteEditor ? '/sites-builder.php' : '/sites-library.php'); break; case 'customers': ?>
  ...
  <?php ez_page_header('Customers', 'View your customers and their purchase history.', [['label'=>'Export customers','toast'=>'Customer export prepared'],['label'=>'Create segment','toast'=>'Segment builder opened','style'=>'primary']]); ?>
  <?php ez_stat_strip([
...
<?php break; case 'analytics': ?>
  <?php ez_page_header('Analytics', 'Track sales, conversion, and product performance in your sandbox.', [['label'=>'Last 6 months','toast'=>'Date range selector opened'],['label'=>'Export report','toast'=>'Analytics report prepared','style'=>'primary']]); ?>
  <?php ez_stat_strip([
      ['icon'=>'money','label'=>'Confirmed revenue','value'=>ez_admin_short_money($metrics['paid_volume']),'detail'=>$metrics['paid_count'].' successful payments'],
      ['icon'=>'trend','label'=>'Conversion','value'=>number_format($conversionRate,1).'%','detail'=>'Paid orders / all orders'],
      ['icon'=>'chart','label'=>'Average order','value'=>ez_admin_short_money($averageOrder),'detail'=>'Confirmed transactions'],
      ['icon'=>'refund','label'=>'Failure rate','value'=>number_format($refundRate,1).'%','detail'=>$metrics['failed_count'].' failed orders'],
  ]); ?>
  <section class="page-grid analytics-hero-grid"><article class="surface analytics-chart"><header class="surface-header"><div><h2>Revenue performance</h2><p>Provider-confirmed revenue across stored order history.</p></div><span class="verified-label"><?= ez_admin_icon('check-circle') ?> Verified data</span></header><div class="large-sales-chart"><div class="axis-y"><span><?= ez_admin_short_money($chartMaximum) ?></span><span><?= ez_admin_short_money($chartMaximum*.5) ?></span><span>Rp0</span></div><svg viewBox="0 0 600 190" preserveAspectRatio="none"><defs><linearGradient id="analytics-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ff4a46" stop-opacity=".3"/><stop offset="1" stop-color="#ff4a46" stop-opacity=".02"/></linearGradient></defs><path class="fill" d="<?= ez_admin_escape($chartArea) ?>"/><path class="stroke" d="<?= ez_admin_escape($chartLine) ?>"/></svg><div class="axis-x"><?php foreach($salesMonths as $month): ?><span><?= ez_admin_escape($month['label']) ?></span><?php endforeach; ?></div></div></article><aside class="surface conversion-funnel"><header class="surface-header"><div><h2>Conversion funnel</h2><p>Checkout completion health.</p></div></header><div class="funnel-steps"><article style="--size:100%"><span>Orders created</span><b><?= $metrics['orders'] ?></b><small>100%</small></article><article style="--size:<?= max(24,$conversionRate+25) ?>%"><span>Payment initiated</span><b><?= $metrics['paid_count']+$metrics['pending_count'] ?></b><small><?= $metrics['orders']>0?number_format((($metrics['paid_count']+$metrics['pending_count'])/$metrics['orders'])*100):0 ?>%</small></article><article style="--size:<?= max(20,$conversionRate) ?>%"><span>Provider confirmed</span><b><?= $metrics['paid_count'] ?></b><small><?= number_format($conversionRate,1) ?>%</small></article></div></aside></section>
  <section class="page-grid analytics-detail-grid"><article class="surface"><header class="surface-header"><div><h2>Product performance</h2><p>Paid units and revenue contribution.</p></div></header><div class="performance-list"><?php $maxProduct=max(1,...array_column($topProducts,'sales')); foreach($topProducts as $name=>$sales): ?><article><?= ez_admin_product_art($sales['name'], $sales['image_url']) ?><div><b><?= ez_admin_escape($sales['name']) ?></b><span><i style="width:<?= round(($sales['sales']/$maxProduct)*100) ?>%"></i></span></div><small><?= $sales['quantity'] ?> units</small><strong><?= ez_admin_short_money($sales['sales']) ?></strong></article><?php endforeach; ?></div></article><article class="surface"><header class="surface-header"><div><h2>Payment quality</h2><p>Provider status distribution.</p></div></header><div class="quality-donut-wrap"><div class="quality-donut" style="--paid:<?= $paidEnd ?>%;--pending:<?= $pendingEnd ?>%;--creating:<?= $creatingEnd ?>%"><span><b><?= $metrics['orders'] ?></b><small>orders</small></span></div><ul><li><i class="q-paid"></i>Paid <b><?= $statusCounts['PAID'] ?></b></li><li><i class="q-pending"></i>Pending <b><?= $statusCounts['PENDING'] ?></b></li><li><i class="q-creating"></i>Creating <b><?= $statusCounts['CREATING'] ?></b></li><li><i class="q-failed"></i>Failed <b><?= $statusCounts['FAILED'] ?></b></li></ul></div></article><article class="surface insight-card"><header class="surface-header"><div><h2>Key insight</h2><p>Automatic performance summary.</p></div></header><div class="insight-body"><span><?= ez_admin_icon('sparkles') ?></span><h3><?= $conversionRate >= 50 ? 'Payment completion is healthy' : 'Checkout recovery is the clearest opportunity' ?></h3><p><?= $metrics['pending_count'] ?> orders remain unresolved, representing <?= ez_admin_short_money($pendingVolume) ?> in potential sandbox volume.</p><a href="?page=marketing">Build a recovery campaign</a></div></article></section>

<?php break; case 'marketing': ?>
  <?php ez_page_header('Marketing', 'Manage your campaigns and customer outreach.', [['label'=>'Campaign calendar','toast'=>'Campaign calendar opened'],['label'=>'New campaign','toast'=>'Campaign builder opened','style'=>'primary']]); ?>
  <?php ez_stat_strip([
  ...
  <section class="campaign-grid"><article class="campaign-card live-campaign"><header><span>Always on</span><em>Active</em></header><div class="campaign-visual campaign-recovery"><?= ez_admin_icon('cart') ?></div><h2>Checkout recovery</h2><p>Remind customers with unresolved payments and return them to checkout.</p><div><span><small>Audience</small><b><?= $metrics['pending_count'] ?> customers</b></span><span><small>Potential</small><b><?= ez_admin_short_money($pendingVolume) ?></b></span></div><footer><button type="button" data-toast="Campaign analytics opened">View performance</button><button type="button" data-toast="Campaign editor opened">Edit</button></footer></article><article class="campaign-card"><header><span>Lifecycle</span><em>Active</em></header><div class="campaign-visual campaign-welcome"><?= ez_admin_icon('sparkles') ?></div><h2>First-order welcome</h2><p>Introduce the catalog to newly identified customers after checkout.</p><div><span><small>Audience</small><b><?= count($customerProfiles) ?> customers</b></span><span><small>Touchpoints</small><b>2 messages</b></span></div><footer><button type="button" data-toast="Campaign analytics opened">View performance</button><button type="button" data-toast="Campaign editor opened">Edit</button></footer></article><article class="campaign-card draft-campaign"><header><span>Retention</span><em>Draft</em></header><div class="campaign-visual campaign-loyalty"><?= ez_admin_icon('star') ?></div><h2>Second-order incentive</h2><p>Encourage one-time customers to return with a curated product recommendation.</p><div><span><small>Audience</small><b><?= count(array_filter($customerProfiles,static fn($p):bool=>$p['orders']===1)) ?> customers</b></span><span><small>Status</small><b>Needs review</b></span></div><footer><button type="button" data-toast="Campaign preview opened">Preview</button><button type="button" data-toast="Campaign editor opened">Finish setup</button></footer></article></section>
  <section class="page-grid automation-grid"><article class="surface"><header class="surface-header"><div><h2>Customer journeys</h2><p>Trigger-based lifecycle automation.</p></div><button type="button" data-toast="Automation builder opened">Create automation</button></header><div class="automation-list"><article><span class="automation-icon"><?= ez_admin_icon('cart') ?></span><div><b>Payment pending</b><p>Order enters PENDING for 30 minutes</p></div><span class="automation-flow"><i></i><i></i><i></i></span><em class="connected">Active</em></article><article><span class="automation-icon"><?= ez_admin_icon('check-circle') ?></span><div><b>Payment confirmed</b><p>Signed provider notification marks PAID</p></div><span class="automation-flow"><i></i><i></i></span><em class="connected">Active</em></article><article><span class="automation-icon"><?= ez_admin_icon('users') ?></span><div><b>First customer order</b><p>Customer profile reaches one paid order</p></div><span class="automation-flow"><i></i><i></i></span><em>Draft</em></article></div></article><aside class="surface channel-card"><header class="surface-header"><div><h2>Channel readiness</h2><p>Available campaign surfaces.</p></div></header><ul><li><span><?= ez_admin_icon('mail') ?></span><div><b>Email</b><small>Customer email captured at checkout</small></div><em class="connected">Ready</em></li><li><span><?= ez_admin_icon('smartphone') ?></span><div><b>WhatsApp</b><small>Phone numbers available</small></div><em>Setup</em></li><li><span><?= ez_admin_icon('monitor') ?></span><div><b>In-app</b><small>Checkout messaging</small></div><em class="connected">Ready</em></li></ul></aside></section>

<?php break; case 'payments': ?>
  <?php ez_page_header('Payments', 'Track payments, pending amounts, and settlement status.', [['label'=>'Refresh','href'=>'?page=payments'],['label'=>'View wallet','href'=>'?page=wallet','style'=>'primary']]); ?>
  <?php ez_stat_strip([
      ['icon'=>'wallet','label'=>'Confirmed volume','value'=>ez_admin_short_money($metrics['paid_volume']),'detail'=>$commerceProduction?'Provider-confirmed volume':'Sandbox payment volume'],['icon'=>'check-circle','label'=>'Successful payments','value'=>(string)$metrics['paid_count'],'detail'=>number_format($conversionRate,1).'% of orders'],['icon'=>'refund','label'=>'Pending volume','value'=>ez_admin_short_money($pendingVolume),'detail'=>$metrics['pending_count'].' unresolved'],['icon'=>'chart','label'=>'Average payment','value'=>ez_admin_short_money($averageOrder),'detail'=>'Provider-confirmed orders'],
  ]); ?>
  <section class="payment-hero"><div><span>DOKU checkout</span><small><?= $commerceProduction ? 'Production' : 'Sandbox' ?> payments</small><strong><?= $integrationStatus['doku'] ? 'Credentials configured' : 'Setup required' ?></strong><p><?= ez_admin_icon('check-circle') ?> Signed payment notifications</p></div><aside><small>Production release</small><b>Acceptance required</b><p>Merchant onboarding, refunds, reconciliation, and verified disbursement remain required before public orders.</p><a href="../../docs/production-commerce-checklist.md">Read production checklist</a></aside></section>
  <section class="page-grid payments-grid"><article class="surface"><header class="surface-header"><div><h2>Transaction ledger</h2><p>Latest provider and checkout states.</p></div><label class="surface-search"><?= ez_admin_icon('search') ?><input data-table-search="payments-table" type="search" placeholder="Search payments" value="<?= ez_admin_escape(mb_substr(is_string($_GET['order'] ?? null) ? $_GET['order'] : '', 0, 100)) ?>"></label></header><div class="ledger-table" id="payments-table"><div class="ledger-head"><span>Reference</span><span>Customer</span><span>Method</span><span>Provider status</span><span>Amount</span></div><?php foreach($allDisplayOrders as $order):$customer=(array)($order['customer']??[]);$status=strtoupper((string)($order['status']??''));?><article data-search-row="<?= ez_admin_escape(mb_strtolower(implode(' ',[(string)($order['order_id']??''),(string)($customer['name']??''),(string)($order['payment_type']??'')]))) ?>"><div><b><?= ez_admin_escape($order['order_id']??'—') ?></b><small><?= ez_admin_escape(ez_admin_time($order['created_at']??'')) ?></small></div><span><?= ez_admin_escape($customer['name']??'Guest') ?></span><span><?= ez_admin_escape(ucwords(str_replace('_',' ',(string)($order['payment_type']??'Awaiting')))) ?></span><span class="status-badge status-<?= ez_admin_escape(strtolower($status)) ?>"><?= ez_admin_escape($order['payment_status']??$order['midtrans_status']??strtolower($status)) ?></span><strong><?= ez_admin_money($order['total']??0) ?></strong></article><?php endforeach; ?><?php if($allDisplayOrders===[]):?><div class="blank-state">Transactions will appear after checkout creates an order.</div><?php endif;?></div></article><aside class="surface methods-panel"><header class="surface-header"><div><h2>Payment methods</h2><p>Method distribution across orders.</p></div></header><div class="method-list"><?php $methodMax=max(1,...array_values($paymentMethods!==[]?$paymentMethods:[1]));foreach($paymentMethods as $method=>$count):?><article><div><b><?= ez_admin_escape($method) ?></b><small><?= $count ?> orders</small></div><span><i style="width:<?= round(($count/$methodMax)*100) ?>%"></i></span><strong><?= number_format(($count/max(1,$metrics['orders']))*100,1) ?>%</strong></article><?php endforeach;?><?php if($paymentMethods===[]):?><div class="blank-state">No payment methods recorded yet.</div><?php endif;?></div><footer><span>Provider environment</span><b><?= $commerceProduction?'Production':'Sandbox' ?></b></footer></aside></section>
...
<?php break; case 'messages': ?>
  <?php ez_page_header('Messages', 'Manage customer conversations and order questions.', [['label'=>'Saved replies','toast'=>'Saved replies opened'],['label'=>'New message','toast'=>'Message composer opened','style'=>'primary']]); ?>
  <?php ez_stat_strip([
      ...
      <section class="surface settings-section" id="security">
        ...
        <div class="security-cards">
          <article class="mfa-security-card">
            ...
            <?php elseif ($mfaSetup === null): ?><form method="post"><input type="hidden" name="action" value="mfa_enroll_start"><input type="hidden" name="csrf_token" value="<?= ez_admin_escape($csrfToken) ?>"><button type="submit">Set up</button></form>
        ...
        <?php if ($mfaSetup !== null): ?>
          <div class="mfa-setup-panel">
            ...
            <form class="mfa-setup-cancel" method="post"><input type="hidden" name="action" value="mfa_enroll_cancel"><input type="hidden" name="csrf_token" value="<?= ez_admin_escape($csrfToken) ?>"><button type="submit">Cancel setup</button></form>
          </div>
        <?php elseif ($mfaEnabled): ?>
          <details class="mfa-disable-panel">
            <summary>Turn off two-step verification</summary>
            <form method="post" autocomplete="off"><input type="hidden" name="action" value="mfa_disable"><input type="hidden" name="csrf_token" value="<?= ez_admin_escape($csrfToken) ?>"><p>Enter your current authenticator code. Your usual sign-in method alone will protect the account afterward.</p><label for="mfa-disable-code">Authenticator code</label><input id="mfa-disable-code" name="code" type="text" inputmode="numeric" pattern="[0-9]{6}" minlength="6" maxlength="6" autocomplete="one-time-code" placeholder="000000" required><button type="submit">Turn off</button></form>
          </details>
        <?php endif; ?>
      </section>
