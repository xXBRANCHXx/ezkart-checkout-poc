(() => {
  ...
  function renderCart() {
    ...
    byId("bag-count").textContent = `${count} ${count === 1 ? "item" : "items"}`;
    ...
    }).join("") : '<p class="shop-cart-empty">Your bag is empty. Add something you love.</p>';
  }
  function updateCard(card) {
    ...
    button.textContent = choice?.available ? "Add to bag" : choice?.stock === 0 && product.type === "physical" ? "Sold out" : "Unavailable";
  ...
  async function load() {
    ...
    try {
      ...
      if (JSON.stringify(saved) !== JSON.stringify(cart)) toast("Your bag was updated to match current availability.");
  ...
  byId("shop-products").addEventListener("click", event => {
    ...
    cart[choice.id] = (cart[choice.id] || 0) + qty; renderCart(); toast(`${product.name} added to your bag`);
  });
