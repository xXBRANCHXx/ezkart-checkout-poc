...
- **Hue (Rona) is ready for native builder review**, at `~/.local/share/ezkart-templates/rona/concept.json`. Its nine sections use ordinary native elements, editable color swatches, a size guide and the shared cart. See `docs/templates/rona-builder-validation.md`. Wait for approval of this native version before packaging it as a template.
- **The professional-business concept is commissioned and running in its own terminal**, in the correct Ezkart workspace with full access. Its working folder is `~/.local/share/ezkart-templates/lajur/`. Treat it as one of the one-or-two more formal templates; retain clean sans-serif typography, whitespace, approachable roundness and composed gradients. Do not launch a duplicate or interfere with that session’s files.
