...
- **Current reconstruction: Lintas (Cross)**, the everyday-bag concept at `~/.local/share/ezkart-templates/lintas/concept.json`. Its native builder recreation is ready for review at `http://127.0.0.1:46352/cart/admin/?page=sites&edit=lintas-native-v1.ezkart.site`. Preserve its reviewed art direction; wait for native approval before template preparation. See `docs/templates/lintas-builder-validation.md`.
- **Next commissioned concept — updated September 16, 2026:** use Ezkart’s own marketing website as the primary visual reference. Make it visibly rounder, with soft gradients, clean sans-serif typography, generous whitespace and an approachable, professional shopping journey. Stop repeating sharp-corner, studio/designer/editorial layouts. This explicit user direction overrides older blanket restrictions on rounded containers and gradient backgrounds; use them with clear hierarchy and readable contrast. Choose a distinct merchant story and composition. Do not start it until commissioned.
...
When the user approves the builder result and asks to make it a template, inspect the repository's current template packaging and application mechanism; use or implement the shared mechanism appropriate to that request. Preserve the editable builder composition, meaningful labels, responsive behavior and asset provenance. Keep design imagery and fictional catalog data separate and declare the catalog requirements. Merchants may start, edit and save a template without selecting products. Licensed design photos may remain on applied pages. Connect real catalog products to shop slots later while preserving design edits. Publishing requires at least one owned, active product with an available purchase action; physical products or visible variants must have positive stock. Validate this on the server, not only the Publish button.
