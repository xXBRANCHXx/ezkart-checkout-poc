...
- **Current reconstruction queue: Hue (Rona)**, the approved concept at `~/.local/share/ezkart-templates/rona/concept.json`. The user commissioned its native builder recreation after the current delivery.
- **Next concept is commissioned:** launch a new terminal for a more professional business design before reconstructing Hue. Treat this as one of the one-or-two more formal templates; retain clean sans-serif typography, whitespace, approachable roundness and composed gradients. Do not repeat the sharp studio/editorial style.
