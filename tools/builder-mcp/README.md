...
The library has 24 editable section compositions: 15 grid compositions in `cart/admin/builder-components.js` and nine responsive compositions adapted from the user-owned Ezkart.id in `cart/admin/builder-showcase-data.js`. The latter preserve the reference’s DOM layout while exposing native editable content groups; they do not embed the page in an iframe. The five navigation compositions are Classic (`studio`), Masthead, Centered (`split`), Shop, and Minimal (`compact`). New headers are sticky by default. The Navigation switch appears at the top of the inspector when the header, logo or links are selected; turning it off keeps the header in normal page flow. Layout changes preserve an explicit opt out. Surface, positioning and scroll behavior remain independently editable under More navigation settings. Tablet and mobile headers use a keyboard-accessible menu, which also opens on wider pages when long links would overlap.
...
- Section background → Gradient exposes linear, radial and soft blends, two colors, base color, angle, strength and radial center. Soft blend also has **Center gradient** and **Gradient height** (10–200%). Turning centering off anchors the blend at the bottom; height changes the painted area without resizing the section. `section_update.gradient` accepts `centered` (default `false`) and `height` (default `100`) alongside the existing values. Adding the first flexible element preserves this appearance. Native sections expose the same controls under Background gradient; choose **Apply background gradient** to save the previewed change. Native fills store these settings in `fill.area: { centered, height, wide? }`; `wide` preserves the wider spread of converted soft blends. Existing native gradients without an area keep their original appearance. The old imported closing gradient migrates into these controls when opened.
