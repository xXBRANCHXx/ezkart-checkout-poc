...
test('reference sections stay editable, preserve device-specific sizing, and export working navigation and media',async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('#top h1').evaluate(n=>n.offsetWidth),1000);assert.equal(await page.locator('[data-ezkart-cart-open]').count(),0);assert.equal(await page.evaluate(()=>[...document.querySelectorAll('h1,h2,h3,p,a,button')].filter(node=>node.getClientRects().length).every(node=>{const style=getComputedStyle(node);return document.fonts.check(`${style.fontStyle} ${style.fontWeight} ${style.fontSize} ${style.fontFamily}`)})),true,'Visible text has its required fonts loaded; unused font weights may remain lazy');
