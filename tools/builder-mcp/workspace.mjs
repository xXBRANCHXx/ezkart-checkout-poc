  ...
  this.server=createServer(async(req,res)=>{
   ...
   try{
    ...
    if(url.pathname.startsWith('/cart/admin/')){
     ...
     if(!file.startsWith(join(repoRoot,'cart/admin/') )||(!/\.(js|css|woff2|webp|png|svg|jpg)$/.test(file)&&!(file.startsWith(join(repoRoot,'cart/admin/templates/'))&&file.endsWith('.json'))))return send(404,{ok:false,error:'File not found.'});
     const types={'.json':'application/json; charset=utf-8','.js':'text/javascript','.css':'text/css','.woff2':'font/woff2','.webp':'image/webp','.png':'image/png','.svg':'image/svg+xml','.jpg':'image/jpeg'};
