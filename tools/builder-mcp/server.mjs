...
tool('element_add','Add a single native element from the same Add panel, without custom markup.',{section:z.string(),type:z.enum(['heading','text','image','button','divider','marquee','navigation']),id:identifier},args=>call('addElement',args));
tool('element_update','Edit a native element. Choose a text field index from page_inspect. Layout coordinates follow page_inspect: grid units for grid elements; pixel offsets and optional pixel width/height for flow elements.',{id:z.string(),text:z.string().optional(),field:z.number().int().min(0).optional(),src:z.string().optional(),alt:z.string().optional(),autoHeight:z.boolean().optional(),headingLevel:z.enum(['h1','h2','h3']).optional(),buttonRole:z.enum(['primary','secondary','tertiary']).optional(),action:z.object({type:z.enum(['none','section','url','email','phone','checkout']),target:z.string().optional(),newTab:z.boolean().optional()}).optional(),inset:z.object({top:z.number().optional(),right:z.number().optional(),bottom:z.number().optional(),left:z.number().optional()}).optional(),style:z.record(z.string(),z.union([z.string(),z.number()])).optional(),layout:z.object({x:z.number().optional(),y:z.number().optional(),width:z.number().optional(),height:z.number().optional()}).optional(),device:device.optional()},args=>call('updateElement',args));
