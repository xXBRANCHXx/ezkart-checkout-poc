...
tool('template_apply','Apply an installed template to a blank page. Products are optional for drafts; connect real products before publishing. All content remains editable through ordinary native controls.',{templateId:identifier,productId:z.string().optional(),productIds:z.array(z.string()).optional(),brandName:z.string().min(1).max(80)},args=>call('applyTemplate',args));
tool('template_products','Connect real catalog products to template slots while preserving design edits.',{productIds:z.array(z.string())},args=>call('connectTemplateProducts',args));
...
tool('native_add','Add individual native containers, rich text, icons, buttons, images, catalog product cards, accordions or video, optionally with nested children. All props have visible inspector controls. HTML, CSS selectors and scripts are not accepted.',{section:identifier,parent:identifier.optional(),node:record,sectionProps:record.optional()},args=>call('nativeInsert',args));
tool('native_update','Change native appearance, inline text ranges, multi-stop gradient fills, custom responsive breakpoints or interaction states using the normal inspector property model.',{id:identifier,props:record.optional(),fill:record.optional(),responsive:z.array(record).optional(),states:record.optional(),productId:z.string().optional(),text:z.string().optional(),marks:z.array(record).optional(),action:record.optional(),src:z.string().optional(),poster:z.string().optional(),captions:z.string().optional(),captionsText:z.string().optional(),open:z.boolean().optional(),fit:record.nullable().optional(),scrollMotion:record.nullable().optional(),scrollVisibility:record.nullable().optional(),productIds:z.array(z.string()).optional(),stateScope:z.string().optional(),initialState:z.string().optional(),stateParam:z.string().optional(),stateMode:z.string().optional(),statePanel:z.string().optional(),part:z.enum(['image','options','price','title','description','product-name','set-price','set-add','quantity','availability','add','cart']).optional(),group:z.string().optional(),optionLayout:z.enum(['compact','detailed','cards','select','swatches']).optional(),variantColors:record.optional(),variantId:z.string().optional(),tableScope:z.enum(['','row','col']).optional(),prefix:z.string().optional(),suffix:z.string().optional(),priceSuffix:z.string().optional(),showPrice:z.boolean().optional(),name:z.string().optional(),alt:z.string().optional(),label:z.string().optional(),icon:z.string().optional(),iconFill:z.string().optional(),iconStroke:z.string().optional(),iconWeight:z.number().optional(),muted:z.boolean().optional(),controls:z.boolean().optional(),loop:z.boolean().optional(),autoplay:z.boolean().optional(),collapsed:z.boolean().optional()},args=>call('nativeUpdate',args));
tool('native_inspect','Read saved native properties, parent IDs, breakpoints, inline text styles and interactions.',{id:identifier.optional()},args=>call('nativeInspect',args),true);
tool('native_move','Move a native element into another container, optionally before a sibling.',{id:identifier,parent:identifier,before:identifier.optional()},args=>call('nativeMove',args));
