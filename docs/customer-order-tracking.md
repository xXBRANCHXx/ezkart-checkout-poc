...
The public projection includes delivery status, bounded timestamped history, courier/service, waybill, HTTPS tracking link, and validated delivery coordinates. Provider keys, full provider responses, addresses and driver contact details are not added to the public API. Order links retain the existing random order reference access model. External links omit referrers; map tiles send only the site origin, never the order query.

## Maps and live tracking

When Biteship supplies both valid pickup and destination coordinates, a Leaflet map shows those two locations. It does not draw an assumed traveled route or a guessed courier position. Missing/invalid coordinates hide the map. Maps initialize only when scrolled into view; map failures leave the timeline usable.
